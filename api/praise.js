...
export default async function handler(req, res) {
  ...
  if (type === 'torah') {
    ...
    const prompt = `אתה מנוע חינוכי יהודי למשחק משפחתי של משפחת ינקו.
צור משפט "הידעת?" אחד לילדים בגיל 5-16.

חוק ברזל: השתמש רק בציטוטים מפורסמים ומוכרים שבאמת קיימים בתורה, בפרקי אבות או בחז"ל.
אל תמציא ציטוטים! אם אינך בטוח — השתמש באחד הציטוטים מהדוגמאות.

דוגמאות לסגנון:
${TORAH_EXAMPLES.slice(0, 3).join('\n')}

מבנה קבוע:
הידעת? "[ציטוט מדויק מהמקור]" – [הסבר קצר בשפה פשוטה לילדים, מילה טובה]
